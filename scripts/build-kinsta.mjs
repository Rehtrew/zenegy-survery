...
// The whole survey lives in one HTML file with no hashed asset names, so a
// browser holding a stale copy runs old code against the live API. Kinsta sends
// no Cache-Control for static files, which lets browsers cache heuristically for
// hours. Serving the page through a tiny PHP file lets us demand revalidation.
await writeFile(join(dist, 'index.php'), `<?php
/**
 * Serves the survey with a cache header. Generated by scripts/build-kinsta.mjs.
 *
 * Everything is inlined in app.html, so there are no hashed filenames to bust a
...
const indexPath = join(dist, 'app.html')
